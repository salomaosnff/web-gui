...
import { compareOrDefault, filterOrDefault, getKeyOrDefault, getLabelOrDefault, getValueOrDefault, SelectOption, useSelect } from './useSelect';
...
<template>
  <UiOverlayMenu v-model:visible="isOpen" full close-on-click :close-on-click-content="!multiple">
    <template #activator="{ attrs }">
      <UiInput v-bind="attrs" class="cursor-pointer" :label :clearable @clear="clear">
        <div class="pt-1 pl-2 overflow-hidden relative">
          <Transition name="ui-select__selection">
            ...
            <template v-else-if="selectedOptions.length === 1" key="unique">
        ...
        <template #append>
          <slot name="append">
            <UiIcon name="mdiChevronDown" class="mr-2 text-5 transition-all" :class="{
              'rotate-180': isOpen
            }" />
          </slot>
        </template>
