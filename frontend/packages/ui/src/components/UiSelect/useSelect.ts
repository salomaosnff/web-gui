...
export function getKeyOrDefault<T>(item: T, index: number, getKey?: (item: T, index: number) => string | number): number | string {
...
export function getValueOrDefault<T, V>(item: T, getKey?: (item: T, index: number) => string | number, getValue?: (item: T) => V): V {
  if (typeof getValue === 'function') {
    return getValue(item)
  }

  if (isObject(item)) {
    return item.id ?? item.value ?? item.key ?? getKeyOrDefault(item, 0, getKey)
  }

  return item as unknown as V
}

export function filterOrDefault<T, V>(option: SelectOption<T, V>, query: string, filter?: (option: SelectOption<T, V>, query: string) => boolean) {
...
export function compareOrDefault<V>(a: V, b: V, compare?: (a: V, b: V) => boolean) {
...
export function useSelect<T, V>(options: UseSelectOptions<T, V>) {
  ...
  const allOptions = computed(() => {
    ...
    const selectOptions = items.map<SelectOption<T, V>>((data, index) => {
      return {
        ...
        value: (isReturnObject ? data : getValueOrDefault(data, options.getKey, options.getValue)) as V,
        get isSelected() {
          return Array.from(valueSet.value).some(value => compareOrDefault(value, this.value, options.compare))
        }
      }
    });
    ...
    if (toValue(options.returnObject)) {
      for (const value of valueSet.value) {
        const option = selectOptions.find(option => compareOrDefault(option.value, value, options.compare))
  ...
  const selectedOptions = computed(() => allOptions.value.filter(option => option.isSelected))
  ...
  const filteredOptions = computed(() => {
    ...
    return allOptions.value.filter(option => filterOrDefault(option, query, options.filter))
  })
